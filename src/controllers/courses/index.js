...
coursesRouter.post('/', upload.single('course_picture'), (req, res) => {
  const { course_title, course_desc, long_desc, estimate_finish_minutes, expertise, will_learning } = req.body;
  ...
  const sql = `
    INSERT INTO courses 
    (course_id, course_title, course_desc, long_desc, estimate_finish_minutes, expertise, course_picture, will_learning, created_at, updated_at) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `;

  try {
    db.query(sql, [course_id, course_title, course_desc, long_desc, estimate_finish_minutes, expertise, imageFilename, will_learning, created_at, updated_at], (err, result) => {
      if (err) {
        console.error(err);
...
coursesRouter.put('/:idCourse', upload.single('course_picture'), (req, res) => {
  ...
  const sql = `
    UPDATE courses 
    SET course_title=?, course_desc=?, long_desc=?, estimate_finish_minutes=?, expertise=?, will_learning=?, updated_at=?
    ${imageFilename ? ', course_picture=?' : ''}
    WHERE course_id=?
  `;

  try {
    const params = [course_title, course_desc, long_desc, estimate_finish_minutes, expertise, will_learning, updated_at];
